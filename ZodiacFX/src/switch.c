...
void task_switch(struct netif *netif)
{
	...
	uint32_t dev_read = gmac_dev_read(&gs_gmac_dev, (uint8_t *) gs_uc_eth_buffer, sizeof(gs_uc_eth_buffer), &ul_rcv_size);
        ...
        /* Only consider a packet good if we have no receive buffers pending after a read.
           Any buffers used after that may be an overrun; unclear whether we can trust
           the tail tag in this situation to count by port. */
